...
namespace KeePassShtokal.Infrastructure
{
    public class MainDbContext:DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            {
                ea.HasOne(x => x.EntryState).WithOne().OnDelete(DeleteBehavior.ClientSetNull);
                ea.HasOne(x => x.Entry).WithOne().OnDelete(DeleteBehavior.ClientSetNull);
                ea.HasOne(x => x.User).WithOne().OnDelete(DeleteBehavior.ClientSetNull);

                ea.Property(e => e.ActionType).HasConversion<string>();
            });

            modelBuilder.Entity<EntryState>(ea =>
            {
                ea.HasOne(x => x.Entry).WithMany(x=>x.EntryStates).OnDelete(DeleteBehavior.ClientSetNull);
            });
